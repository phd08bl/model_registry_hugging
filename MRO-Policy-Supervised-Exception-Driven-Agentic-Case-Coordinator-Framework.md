...
It contains three related designs:
...
# Generic MRO Agentic Workflow Framework: More Explaination

## 1. Executive summary

The **Generic MRO Agentic Workflow Framework** is designed as an agentic-capable architecture. A specific implementation qualifies as an Agentic AI workflow when the Coordinator genuinely uses the current Case state to:

* identify unresolved objectives;
* select different permitted actions;
* invoke appropriate tools;
* verify results;
* update authoritative state;
* replan when information changes; and
* continue until it must wait, escalate to a human or complete.

It is not Agentic AI merely because it uses an LLM. The agentic behaviour comes from state-aware coordination, goal-directed action, governed tool use, verification and replanning.

The recommended design is:

> **One Policy-Supervised, Exception-Driven Case Coordinator using multiple governed tools and capabilities.**

It is reusable across many MRO applications because those applications often share the same underlying Case-management pattern, even though their domain rules, tools and decisions are different.

---

## 2. Why it qualifies as Agentic AI

### 2.1 It works towards a bounded objective

The Coordinator operates against a clearly defined Case objective rather than simply moving through a sequence of screens.

For example, the AI Risk Triage Coordinator’s objective is:

> Prepare a complete, transparent and evidence-linked risk-triage proposal for AIRO review, without independently changing the final AIRO-owned risk decision.

The objective is explicit, measurable and constrained. The Coordinator cannot reinterpret or expand it.

### 2.2 It maintains persistent Case state

Its memory is not simply a chat history. The authoritative Case state records:

* confirmed and candidate facts;
* evidence, provenance and citations;
* missing information and conflicts;
* unresolved objectives;
* completed, pending and failed actions;
* human decisions and rationale;
* current and superseded results;
* policy, rule, model and tool versions;
* the effective automation profile; and
* pending human or external events.

Each action is therefore based on the current Case state rather than starting from scratch.

### 2.3 It dynamically selects the next permitted action

A fixed workflow normally follows a predefined sequence:

```text
Step 1 → Step 2 → Step 3 → Step 4 → Complete
```

The Agentic Case Coordinator instead determines the next action from the Case state.

For example:

```text
Case A: Evidence is complete
→ Run the materiality and 2LoD engines

Case B: Supplier evidence is missing
→ Prepare an evidence request
→ Wait for the stakeholder response

Case C: Questionnaire and evidence conflict
→ Run an evidence-consistency check
→ Escalate the verified conflict to AIRO
```

Different Cases can therefore follow different paths without requiring the user to manually navigate the workflow.

### 2.4 It uses governed tools

The Coordinator can select and invoke approved capabilities such as:
...
* 2LoD trigger assessment;
* validation evaluations;
* report generation; and
* Confluence or other system adapters.

These capabilities are tools used by the Coordinator. They should not be implemented as multiple independent agents unless there is a clear requirement for separate autonomy and accountability.

The preferred design is:

> **One Coordinator + multiple governed tools and capabilities**

### 2.5 It verifies results before accepting them

A conventional workflow may accept a tool or LLM output immediately. This framework introduces an explicit verification step:

```text
Tool result
→ Validate schema and tool contract
→ Check evidence and citations
→ Check permissions and relevance
→ Check confidence and conflicts
→ Update authoritative Case state only if verified
```

If an LLM result cannot be fully verified, it should remain clearly labelled:

> **Advisory observation — human confirmation required.**

This prevents an unsupported LLM output from becoming an authoritative MRO fact or conclusion.

### 2.6 It replans when the Case changes

If new information changes a material fact, the Coordinator identifies which previous outputs are now stale.

For example:

```text
New evidence confirms use of personal data
→ Update the personal-data fact
→ Invalidate affected evidence checks
→ Invalidate the current materiality proposal
→ Invalidate relevant 2LoD routing
→ Invalidate the review pack
→ Rerun only the affected work
→ Escalate if the protected outcome changes
```

This selective replanning is an important agentic capability. The Coordinator does not blindly restart the entire workflow or leave outdated results appearing current.

### 2.7 It handles events, waiting and resumption

The Coordinator can pause while waiting for:

* stakeholder evidence;
* a specialist 2LoD response;
* model access;
* validation data;
* an evaluation job;
* remediation evidence; or
* an authorised human decision.

A verified event resumes the same Case from its persisted state.

### 2.8 It has bounded autonomy

The Coordinator may be authorised to:

* perform read-only checks;
* choose the next evidence-processing action;
* invoke deterministic engines;
* update non-protected Case state;
* generate draft review packs; and
* identify and escalate exceptions.

It cannot independently:

* expand its permissions;
* increase its automation profile;
* modify approved rules;
* bypass mandatory human governance;
* make protected MRO decisions;
* approve an exception; or
* publish a formal record without authority.

It is therefore best described as:

> **A policy-supervised, decision-constrained and action-constrained Agentic AI workflow.**

---

## 3. How planning works

The framework uses two controlled planning mechanisms.

### 3.1 Deterministic Policy Supervisor

The Policy Supervisor establishes the safe operating boundary for each cycle. It determines:

* permitted and prohibited actions;
* available tools;
* mandatory actions;
* whether the LLM recommender may be used;
* whether human judgement is required;
* action, retry and loop limits;
* external-write permissions; and
* the effective Progressive Automation profile.

The Policy Supervisor is deterministic, version-controlled and testable. It must not use an LLM to grant authority.

### 3.2 Bounded LLM Action Recommender

When several permitted evidence or analytical actions are available, the LLM may recommend which action is most useful.
...
```json
{
  "selected_action": "CHECK_EVIDENCE_CONSISTENCY",
  "selected_tool": "evidence_consistency_checker",
  "reason": "The questionnaire states that no personal data is used, but the evidence refers to employee email addresses.",
  "confidence": 0.93
}
```

The recommendation is then checked by the deterministic Action Authorisation Validator before execution.

The resulting control model is:

```text
Deterministic policy defines the permitted action space
→ LLM recommends within that bounded space
→ Deterministic control authorises or rejects the recommendation
→ Approved tool executes
→ Independent verifier checks the result
```

This is more suitable for MRO than allowing an LLM to create and execute an unrestricted plan.

---

## 4. Why it is reusable across many MRO tools

Many MRO applications have different business purposes but share a common Case-management structure:

```text
Create a Case
→ Collect information and evidence
→ Identify missing or conflicting information
→ Apply approved rules or methodology
→ Invoke specialist tools
→ Prepare a proposed outcome
→ Escalate exceptions and judgement points
→ Obtain an authorised human decision
→ Produce a formal record
→ Retain a complete audit trail
```

This common structure makes it possible to reuse the same Coordinator Kernel.

| Common requirement   | AI risk triage                              | Independent validation                   | Other MRO applications              |
| -------------------- | ------------------------------------------- | ---------------------------------------- | ----------------------------------- |
| Authoritative state  | Questionnaire, evidence and triage results  | Model, data, tests and findings          | Documents, issues and remediation   |
| Open objectives      | Obtain evidence and complete assessment     | Complete testing and reporting           | Resolve gaps or findings            |
| Deterministic policy | Triage and AIRO rules                       | Validation methodology                   | Review and escalation rules         |
| Governed tools       | Evidence, materiality and 2LoD tools        | Scenario, evaluation and report tools    | Document review or monitoring tools |
| LLM support          | Evidence extraction and follow-up questions | Scenario generation and failure analysis | Summarisation, mapping and drafting |
| Human judgement      | AIRO decision                               | Validator conclusion                     | Reviewer or risk-owner decision     |
| External events      | Use-case-owner or 2LoD response             | Data, access or evaluation completion    | Remediation or monitoring evidence  |
| Formal output        | AIRO review pack and triage record          | Independent validation report            | Review record or issue closure      |
...
* finding and issue remediation;
* evidence collection;
* ongoing monitoring and escalation;
* rule calibration and backtesting; and
* model-change assessment.

---

## 5. What is reusable and what remains application-specific

The framework should not become one universal agent that performs every MRO activity.

### Reusable Coordinator Kernel
...
→ Update
→ Replan
→ Continue, Wait, Escalate or Complete
```

It can provide common capabilities for:
...
Each MRO application must still define:

* its bounded objective;
* authoritative facts and evidence;
* completion criteria;
* protected human decisions;
* deterministic rules and methodology;
* approved actions and tools;
* verification requirements;
* exception and escalation rules;
* human Governance Loops;
* Progressive Automation eligibility;
* external-event types; and
* output and publication requirements.

For example:

| Generic component     | AI risk triage                         | Independent validation                           |
| --------------------- | -------------------------------------- | ------------------------------------------------ |
| Policy Supervisor     | Triage permissions and AIRO Gates      | Methodology, mandatory tests and validator Gates |
| Deterministic engines | Materiality and 2LoD routing           | Metrics, thresholds and mandatory-test rules     |
| Human Governance      | AIRO judgement                         | Validator judgement                              |
| Tool Registry         | Evidence, triage and publication tools | Model, data, evaluation and reporting tools      |
| Completion criteria   | Confirmed triage record                | Approved validation conclusion and report        |

---

## 6. Why not build a separate workflow for every MRO tool?

A common framework provides:

* consistent Human Governance;
* consistent policy and permission controls;
* standard tool contracts;
* consistent state and event handling;
* consistent Progressive Automation controls;
* standard exception and fail-safe handling;
* common audit and observability;
* easier testing and validation;
* simpler architecture approval;
* more understandable code for the team; and
* easier expansion through additional domain tools and policies.

A practical structure could be:

```text
Common Agentic Coordinator Kernel
├── AI Risk Triage configuration
├── Independent Validation configuration
├── Documentation Review configuration
├── Issue Remediation configuration
└── Monitoring and Escalation configuration
```

The applications share the control architecture while retaining their own domain rules, tools, evidence and human decision rights.

---

## 7. Progressive Automation remains system-assigned

The Coordinator must not choose its own level of autonomy. The Case user and LLM must not select or increase the production automation profile.

A deterministic policy assigns the effective profile:

| Profile              | Coordinator authority                                                                                             |
| -------------------- | ----------------------------------------------------------------------------------------------------------------- |
| `HUMAN_GOVERNED`     | Prepares and recommends; applicable protected human decisions remain mandatory                                    |
| `CONDITIONAL_REVIEW` | Approved preparation actions may run automatically; defined final decisions remain human-owned                    |
| `EXCEPTION_BASED`    | Proven patterns continue automatically unless an exception, elevated risk or sampling requirement triggers review |
| `STRAIGHT_THROUGH`   | Only explicitly approved low-risk patterns may complete specifically authorised actions automatically             |

The system may automatically reduce autonomy when risk, uncertainty or control failure increases. It must not silently increase autonomy beyond approved eligibility.

---

## 8. It is not appropriate for every MRO application

A complete Agentic Case Coordinator may not be necessary for:

* a simple calculator;
* a static dashboard;
* a one-time policy lookup;
* a fixed-format converter;
* a short process with no meaningful branching; or
* a deterministic function with no event, replanning or human-governance requirement.

In those cases, a simpler design may be sufficient:

```text
User interface → Deterministic function → Result
```

The team can use five questions to assess whether an Agentic Case Coordinator is justified:

1. Do different Cases require different next actions?
2. Can new evidence invalidate previous results?
3. Must several tools be coordinated to complete the objective?
4. Does the process need to wait for and resume from human or external events?
5. Are there exceptions or final decisions that must remain with an authorised MRO colleague?

If most answers are yes, the Agentic Case Coordinator pattern is likely to provide value.

---

## 9. Recommended team positioning

The framework should not be presented as a requirement to turn every MRO tool into an agent.

The recommended positioning is:

> The MRO Policy-Supervised, Exception-Driven Agentic Case Coordinator is a reusable architecture pattern for complex, variable and evidence-driven MRO Case work. It coordinates state, objectives, approved tools, verification, replanning and human governance. Each MRO application retains its own methodology, rules, evidence requirements and accountable human decisions.

It is reusable across many MRO tools because those tools share a governed Case-management structure—not because their business logic is the same.
...
The **AI Risk Triage Case Coordinator** applies the generic framework to all submitted AI use cases.
